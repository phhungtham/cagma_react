...
const SelectTermsBottom = ({
  ...
  return (
    <>
      <BottomSheet open={true} onClose={handleCloseBottomSheet} title="Select Terms" type="fit-content" clazz="select-terms__bottomsheet">
        <div className="form__wrapper">
          <input ref={inputRef} onChange={onChangeTerm} className='select-terms__input-hidden' type='number' onBlur={onBlurInput} maxLength={3} />
          ...
          {isInvalidValue && <div className='select-terms__error-alert'>
            <span className='error-icon mr-1'><ErrorIcon /></span>
            <span>Please enter less than {max} {typeWithUnitLabel[type]}</span>
          </div>}
          <div className='select-terms__options'>
            <Chips segments={options} onChange={onSelectChipTerm} value={termValue} />
          </div>
          ...
          <div className='btn-submit__wrapper'>
            <Button label="Next" variant="filled__primary" className="btn-submit" onClick={onClickConfirm} disable={isInvalidValue || !termValue} />
          </div>
        </div>
      </BottomSheet>
    </>
