      ...
      <div>
        <div className='select_date_wrapper'>
          {type !== selectType.year && <div ref={scrollMonthViewRef} className='select_item_wrapper'>
            {months.map((item, idx) => (
              <div key={`month - ${item}`}
                className={`select_item ${currentMonth === idx ? 'selected' : ''} ${currentMonth - idx === 2 ? 'rotateTop' : (idx - currentMonth) === 2 ? 'rotateBottom' : ''}`}
          ...
          <div ref={scrollYearViewRef} className='select_item_wrapper'>
            {years.map((item) => (
              <div key={`year - ${item} `}
        ...
        <div className="btn_container">
          <Button label="Confirm" variant="filled__primary" className="w_full" onClick={handleConfirmSelectedDate} />
        </div>
      </div>
